...
const CharacterPage = () => {
  ...
  // Re-run load when a login just happened from modal/panel
  useEffect(() => {
    if (user && mode === "require-login") {
      const run = async () => {
        try {
          const { data } = await API.get("/characters/me");
          setCharacter(data);
  ...
  const handleCloseSelector = async (success) => {
    // onClose(success) from selector: when success===true, refresh and show success;
    // when success===false (user clicked X), go back to profile or home
    if (success) {
      try {
        const me = await API.get("/auth/me");
        login(me.data);
      ...
      // Cancel selection: go back to profile if user has a character, otherwise go home
      try {
        const { data } = await API.get("/characters/me");
        setCharacter(data);
        setMode("profile");
  ...
  if (mode === "require-login") {
    return <RequireLogin title="Please log in to select a character" />;
  }
