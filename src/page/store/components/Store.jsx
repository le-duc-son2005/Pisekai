...
import { Container, Row, Col, Card, Button, Form, InputGroup, Modal, Image, Alert, Spinner } from "react-bootstrap";
...
import { useStoreQuery } from "../hook/useStoreQuery.js";
import { purchaseStoreItem } from "../open-api/storeApi.js";
...
const Store = () => {
    ...
    const [buyMsg, setBuyMsg] = useState(null); // { type: 'success'|'danger', text }
    ...
    const closePurchase = () => { setShowPurchase(false); setBuyMsg(null); };
    ...
    const confirmBuy = async () => {
        ...
        setBuyMsg(null);
        try {
            const result = await purchaseStoreItem({ itemId: selectedItem.id, quantity: modalQty });
            setBuyMsg({ type: "success", text: `${result.message} ${result.quantity}x ${result.itemName} (${result.totalCost.toLocaleString("vi-VN")} ${result.currency})` });
        } catch (err) {
            setBuyMsg({ type: "danger", text: err?.response?.data?.message || err.message || "Mua thất bại" });
        } finally {
            setBuying(false);
        }
    };
                    ...
                    {selectedItem && (
                        <>
                            ...
                            <Modal.Body className="pm-body">
                                ...
                                {buyMsg && (
                                    <Alert variant={buyMsg.type} className="mt-3 mb-0 py-2 text-center small">
                                        {buyMsg.text}
                                    </Alert>
                                )}
                            </Modal.Body>
                            <Modal.Footer className="pm-footer">
                                <Button className="pm-cancel-btn" variant="secondary" onClick={closePurchase} disabled={buying}>Cancel</Button>
                                <Button className="pm-buy-btn" variant="dark" onClick={confirmBuy} disabled={buying}>
                                    {buying ? <><Spinner animation="border" size="sm" className="me-1" />Đang mua...</> : "Buy"}
                                </Button>
                            </Modal.Footer>
                        </>
                    )}
